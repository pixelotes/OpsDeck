...
def seed_data():
    ...
    with app.app_context():
        ...
        db.session.commit() # Commit here to ensure suppliers get IDs
        ...
        # Use add_all which is safer for relationships than bulk_save_objects
        contacts = [
        ...
        assets = [
            Asset(name='DEV-LT-001', brand='Dell', model='XPS 15', serial_number=fake.uuid4(), status='In Use', purchase=purchase2, user=users[0], location=locations[0], supplier=suppliers[2], price=2500, warranty_length=36),
            Asset(name='DEV-LT-002', brand='Dell', model='XPS 15', serial_number=fake.uuid4(), status='In Use', purchase=purchase2, user=users[2], location=locations[0], supplier=suppliers[2], price=2500, warranty_length=36),
            Asset(name='DSN-LT-001', brand='Apple', model='MacBook Pro 16"', serial_number=fake.uuid4(), status='In Use', purchase=purchase4, user=users[3], location=locations[1], supplier=suppliers[6], price=3200, warranty_length=24),
            Asset(name='SALES-LT-001', brand='Microsoft', model='Surface Laptop 5', serial_number=fake.uuid4(), status='In Storage', location=locations[0], supplier=suppliers[1], price=1800, warranty_length=24),
        ...
        peripherals = [
            Peripheral(name='Keyboard-001', type='Keyboard', serial_number=fake.uuid4(), asset=assets[0]),
            Peripheral(name='Mouse-001', type='Mouse', serial_number=fake.uuid4(), asset=assets[0]),
            Peripheral(name='Monitor-001', type='Monitor', serial_number=fake.uuid4(), asset=assets[0]),
            Peripheral(name='Keyboard-002', type='Keyboard', serial_number=fake.uuid4(), asset=assets[1]),
            Peripheral(name='Mouse-002', type='Mouse', serial_number=fake.uuid4(), asset=assets[1]),
            Peripheral(name='Monitor-002', type='Monitor', serial_number=fake.uuid4(), asset=assets[1]),
            Peripheral(name='Webcam-CONF-NYC', type='Webcam', serial_number=fake.uuid4(), asset=assets[4]),
            Peripheral(name='Apple Pencil', type='Stylus', serial_number=fake.uuid4(), asset=assets[6]),
        ]
        ...
        services_data = [
            {'name': 'Adobe Creative Cloud', 'type': 'Software', 'renewal': date(2025, 11, 1), 'period': 'yearly', 'cost': 15000, 'currency': 'EUR', 'supplier': suppliers[0], 'tags': [tags[0], tags[6]]},
            {'name': 'Microsoft 365 E5', 'type': 'SaaS', 'renewal': date(2026, 1, 1), 'period': 'yearly', 'cost': 35000, 'currency': 'USD', 'supplier': suppliers[1], 'tags': [tags[0]]},
            {'name': 'Slack Business+', 'type': 'SaaS', 'renewal': date.today() + timedelta(days=20), 'period': 'monthly', 'cost': 1200, 'currency': 'EUR', 'supplier': suppliers[3], 'tags': [tags[0]]},
            {'name': 'Jira & Confluence Cloud', 'type': 'SaaS', 'renewal': date(2026, 1, 5), 'period': 'yearly', 'cost': 25000, 'currency': 'USD', 'supplier': suppliers[4], 'tags': [tags[0], tags[3]]},
            {'name': 'Zoom Business', 'type': 'SaaS', 'renewal': date.today() + timedelta(days=45), 'period': 'yearly', 'cost': 2000, 'currency': 'EUR', 'supplier': suppliers[5], 'tags': [tags[0]]},
            {'name': 'AWS Hosting', 'type': 'Cloud', 'renewal': date(date.today().year, date.today().month, 1), 'period': 'monthly', 'cost': 500, 'currency': 'USD', 'supplier': suppliers[8], 'tags': [tags[5], tags[3]]},
            {'name': 'Figma Organization Plan', 'type': 'SaaS', 'renewal': date(2026, 3, 1), 'period': 'yearly', 'cost': 8000, 'currency': 'USD', 'supplier': suppliers[10], 'tags': [tags[0], tags[6]]},
            {'name': 'companydomain.com', 'type': 'Domain', 'renewal': date(2027, 8, 15), 'period': 'yearly', 'cost': 15, 'currency': 'USD', 'supplier': suppliers[9], 'tags': []},
        ]
